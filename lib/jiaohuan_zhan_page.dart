...
class JiaohuanZhanPage extends StatefulWidget {
  ...
  const JiaohuanZhanPage({super.key, required this.levelId});
  @override
...
class _JiaohuanZhanPageState extends State<JiaohuanZhanPage> {
  ...
  Widget _buildHeader(Color color) {
    return Column(
      children: const [
        Text('交换站', style: TextStyle(fontSize: 32, fontWeight: FontWeight.bold, color: Colors.white, letterSpacing: 8, fontFamily: 'monospace', shadows: [Shadow(color: Color(0xFF6CE4FF), blurRadius: 20)])),
        SizedBox(height: 12),
        Text('SWITCH STATION v3.4', style: TextStyle(fontSize: 10, color: Color(0x666CE4FF), letterSpacing: 2, fontFamily: 'monospace')),
      ],
    );
  }
        ...
        child: Stack(
          children: [
            ...
            Column(mainAxisSize: MainAxisSize.min, children: [
              Row(children: [Icon(Icons.sync_alt, size: 16, color: color), const SizedBox(width: 8), Text("// SWITCH_CHANNEL", style: TextStyle(color: color.withValues(alpha: 0.6), fontSize: 10, fontFamily: 'monospace', letterSpacing: 2)), const Spacer(), Text("SESSION", style: TextStyle(color: color.withValues(alpha: 0.6), fontSize: 10, fontFamily: 'monospace', letterSpacing: 2))]),
              const SizedBox(height: 12),
  ...
  Widget _cardTile(CardData c, int i) {
    return Padding(
      ...
      child: SizedBox(
        ...
        child: Stack(
          children: [
            Positioned.fill(child: gameCardWidget(c, onTap: () => _select(c, _prices[i]))),
            Positioned(
              top: 6,
              left: 6,
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(color: const Color(0xFFFF6A6A).withValues(alpha: 0.6)),
                ),
                child: Row(
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    const Icon(Icons.favorite, size: 12, color: Color(0xFFFF6A6A)),
                    const SizedBox(width: 4),
                    Text('HP -${_prices[i]}', style: const TextStyle(color: Color(0xFFFF6A6A), fontSize: 9, fontFamily: 'monospace')),
                  ],
                ),
              ),
            ),
...
class _DepotGridPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    final paint = Paint()..color = const Color(0xFF42A5F5).withValues(alpha: 0.05)..strokeWidth = 1;
    const spacing = 30.0;
