...
class ShangpuPage extends StatefulWidget {
  ...
  const ShangpuPage({super.key, required this.levelId});
  @override
...
class _ShangpuPageState extends State<ShangpuPage> {
  ...
  Widget _buildHeader(Color color) {
    return Column(
      children: const [
        Text('商铺', style: TextStyle(fontSize: 32, fontWeight: FontWeight.bold, color: Colors.white, letterSpacing: 8, fontFamily: 'monospace', shadows: [Shadow(color: Color(0xFF6CE4FF), blurRadius: 20)])),
        SizedBox(height: 12),
        Text('SHOP PROTOCOL v3.4', style: TextStyle(fontSize: 10, color: Color(0x666CE4FF), letterSpacing: 2, fontFamily: 'monospace')),
      ],
    );
  }
        ...
        child: Stack(
          children: [
            ...
            Column(mainAxisSize: MainAxisSize.min, children: [
              Row(children: [Icon(Icons.store, size: 16, color: color), const SizedBox(width: 8), Text("// SHOP_CHANNEL", style: TextStyle(color: color.withValues(alpha: 0.6), fontSize: 10, fontFamily: 'monospace', letterSpacing: 2)), const Spacer(), Text("SESSION", style: TextStyle(color: color.withValues(alpha: 0.6), fontSize: 10, fontFamily: 'monospace', letterSpacing: 2))]),
              const SizedBox(height: 12),
            ...
            Positioned(
              ...
              child: Container(
                ...
                child: Row(
                  ...
                  children: const [
                    Icon(Icons.local_offer, size: 12, color: Color(0xFFFFA726)),
                    SizedBox(width: 4),
                    Text('促销', style: TextStyle(color: Color(0xFFFFA726), fontSize: 9, fontFamily: 'monospace')),
                  ],
                ),
              ),
            ),
...
class _DepotGridPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    final paint = Paint()..color = const Color(0xFFFFA726).withValues(alpha: 0.05)..strokeWidth = 1;
    const spacing = 30.0;
