...
class JiaoyidianPage extends StatefulWidget {
  ...
  const JiaoyidianPage({super.key, required this.levelId});
  @override
...
class _JiaoyidianPageState extends State<JiaoyidianPage> {
  ...
  Widget _buildHeader(Color color) {
    return Column(
      children: const [
        Text('交易点', style: TextStyle(fontSize: 32, fontWeight: FontWeight.bold, color: Colors.white, letterSpacing: 8, fontFamily: 'monospace', shadows: [Shadow(color: Color(0xFF6CE4FF), blurRadius: 20)])),
        SizedBox(height: 12),
        Text('EXCHANGE NODE v3.4', style: TextStyle(fontSize: 10, color: Color(0x666CE4FF), letterSpacing: 2, fontFamily: 'monospace')),
      ],
    );
  }
        ...
        child: Stack(
          children: [
            ...
            Column(mainAxisSize: MainAxisSize.min, children: [
              Row(children: [Icon(Icons.swap_horiz, size: 16, color: color), const SizedBox(width: 8), Text("// EXCHANGE_CHANNEL", style: TextStyle(color: color.withValues(alpha: 0.6), fontSize: 10, fontFamily: 'monospace', letterSpacing: 2)), const Spacer(), Text("SESSION", style: TextStyle(color: color.withValues(alpha: 0.6), fontSize: 10, fontFamily: 'monospace', letterSpacing: 2))]),
              const SizedBox(height: 12),
            ...
            Positioned(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(color: const Color(0xFF26A69A).withValues(alpha: 0.6)),
                ),
                child: Row(
                  mainAxisSize: MainAxisSize.min,
                  children: const [
                    Icon(Icons.swap_horiz, size: 12, color: Color(0xFF26A69A)),
                    SizedBox(width: 4),
                    Text('撮合', style: TextStyle(color: Color(0xFF26A69A), fontSize: 9, fontFamily: 'monospace')),
                  ],
                ),
              ),
            ),
  ...
  Future<void> _select(CardData c) async {
    ...
    if (mounted) {
      CyberToast.show(context, '已从交易点获取卡牌 [${c.name}]');
...
class _DepotGridPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    final paint = Paint()..color = const Color(0xFF26A69A).withValues(alpha: 0.05)..strokeWidth = 1;
    const spacing = 30.0;
